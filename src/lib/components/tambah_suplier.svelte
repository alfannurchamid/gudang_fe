<script>
  ...
  const upload_suplier = async () => {
    await cek_token();

    const nomor_ahir = data.length;
    nama_suplier = nama_suplier.toUpperCase();
    asal_negara = asal_negara.toUpperCase();
    ...
    const response = await fetch(
      "http://127.0.0.1:8000/api/api/v1/suplier/add_suplier",
      {
        method: "POST",
        headers: {
          accept: "*/*",
...
>
  ...
  >
    ...
    >
      ...
      <h1 class="text-center text-xs font-semibold">Tambah Suplier</h1>
    ...
    <div class="w-full md:flex-row flex-col flex p-5">
      <form class="submitted w-full" on:submit|preventDefault={upload_suplier}>
        <div class=" md:flex-col flex-row w-full flex pr-4 items-center mr-4">
          <div class="flex w-10/12 flex-col">
            <div class=" flex flex-row w-full justify-start">
              <h5 class=" py-3 mr-10 text-gray-600">nama suplier :</h5>
              <input
                bind:value={nama_suplier}
                type="text"
                class="inputB uppercase w-9/12 my-3 px-2 rounded-lg"
                required
              />
            </div>
            <div class=" flex flex-row w-full justify-start">
              <h5 class=" py-3 mr-10 text-gray-600">asal negara :</h5>
              <input
                bind:value={asal_negara}
                type="text"
                class="inputB my-3 px-2 uppercase w-2/12 rounded-lg"
                required
