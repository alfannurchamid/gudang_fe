<script>
  ...
  /**
   * @type {{ nomor: string; jenis:boolean, jenis_dokumen:string, tanggal:string,done:boolean}[]}
   */
  export let data;
  ...
  let jenis_dokumen = "";
  let jenis = false;
  let jenis_dok_lis_in = ["", "BC.23", "BC.27"];
  let jenis_dok_lis_out = ["", "BC.25", "BC.27"];
  let tanggal = "";
  ...
  const upload_barang = async () => {
    ...
    const response = await fetch(
      "http://127.0.0.1:8000/api/api/v1/standar_doc/add_doc",
      {
        method: "POST",
        headers: {
          accept: "*/*",
...
>
  ...
  >
    ...
    >
      ...
      <h1 class="text-center text-xs font-semibold">Tambah Dokumen</h1>
        ...
        <div class=" md:flex-col flex-row w-full flex pr-4 items-center mr-4">
          <div class="flex w-10/12 flex-col">
            <div class=" flex flex-row w-full justify-start">
              <h5 class=" py-3 mr-10 text-gray-600">jenis :</h5>
              <select
            ...
            <div class=" flex flex-row w-full justify-start">
              <h5 class=" py-3 mr-10 text-gray-600">jenis dokumen :</h5>
              ...
              >
                {#if jenis}
                  {#each jenis_dok_lis_in as dt_b}
                    <option value={dt_b}> {dt_b}</option>
                  {/each}
                {:else}
                  {#each jenis_dok_lis_out as dt_b}
                    <option value={dt_b}> {dt_b}</option>
                  {/each}
                {/if}
              </select>
            ...
            <div class=" flex flex-row w-full justify-start">
              <h5 class=" py-3 mr-10 text-gray-600">nomor pendaftaran :</h5>
              <input
                maxlength="6"
                bind:value={nomor}
                type="text"
            ...
            <div class=" flex flex-row w-full justify-start">
              <h5 class=" py-3 mr-10 text-gray-600">tanggal :</h5>
              <input
                bind:value={tanggal}
                type="date"
                class="inputB my-3 px-2 uppercase w-9/12 rounded-lg"
