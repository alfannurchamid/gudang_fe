...
<div
	class="fixed top-0 left-0 right-0 flex z-30 justify-between h-10 w-full pt-5 md:bg-opacity-60 px-5"
>
	<div class="w-1/3 px-2 md:px-10 flex items-center">
	...
	<div class="w-1/3 flex items-center justify-end px-2 md:px-10">
		<!-- svelte-ignore a11y-click-events-have-key-events -->
		<div class="bg-white p-0.5 px-3 rounded-3xl flex">
			<h1 class="text-gray-400 text-sm">dicetak oleh {$user?.username}</h1>
			<div class="w-5 pt-0.5 mx-1">
				<svg
					id="iconUser"
					class="transition"
			...
			{#if $user?.access == 0}
				<h1 class="text-gray-400 text-sm">BC</h1>
			{:else if $user?.access == 1}
				<h1 class="text-gray-400 text-sm">admin</h1>
			{:else}
				<h1 class="text-gray-400 text-sm">super admin</h1>
			{/if}
		</div>
	</div>
</div>
